...
import {
  Eye, EyeOff, Github, Lock, Mail, ArrowRight, Chrome, User, IdCard,
} from "lucide-react";
import { supabaseBrowser } from "@/lib/supabase-client";

type Mode = "login" | "signup";

export default function LoginCardSection({ motivo }: { motivo?: string }) {
  ...
  const [err, setErr] = useState<string | null>(
    motivo === "perfil-eliminado"
      ? "Tu perfil fue eliminado por un administrador. Contactá al Coordinador."
      : null,
  );
  ...
  async function submit(e: React.FormEvent) {
    ...
    start(async () => {
      ...
      if (mode === "login") {
        ...
        // Si hay auto-login (email confirmation off), entra directo
        if (data.session) return router.replace("/panel");
        setErr("Cuenta creada. Revisá tu email para confirmar y volvé a ingresar.");
        setMode("login");
      }
    });
  }
  ...
  async function oauth(provider: "github" | "google") {
    ...
    if (error) setErr(`${provider} no configurado en Supabase: ${error.message}`);
  }
  ...
  return (
    <section className="fixed inset-0 bg-zinc-950 text-zinc-50 overflow-y-auto">
      ...
      <header className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-zinc-800/80 bg-zinc-950/50 backdrop-blur">
        <span className="text-xs tracking-[0.14em] uppercase text-zinc-400">
          Tecnovich · Sistema de Reservas
        </span>
        <a href="https://github.com/nachoborgonovo08/tecnovich" target="_blank" rel="noreferrer">
      ...
      <div className="h-full w-full grid place-items-center px-4 py-24">
        <Card className="card-animate w-full max-w-sm border-zinc-800 bg-zinc-900/70 backdrop-blur supports-[backdrop-filter]:bg-zinc-900/60">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-zinc-400">
              {mode === "login"
                ? "Ingresá al Sistema de Reservas para gestionar materiales y talleres."
                : "Registrate para empezar a reservar materiales y talleres."}
            </CardDescription>
          </CardHeader>
            ...
            <form onSubmit={submit} className="grid gap-5">
                ...
                <>
                  <div className="grid gap-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        id="nombre" type="text" required minLength={2}
                        value={nombre}
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="rol" className="text-zinc-300">Rol</Label>
                    <div className="relative">
                      ...
                      <select
                        ...
                        className="flex h-10 w-full rounded-md border border-zinc-800 bg-zinc-950 pl-10 pr-3 py-2 text-sm text-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                      >
              ...
              {mode === "login" && (
                <div className="flex items-center justify-between">
                  ...
                  <a href="#" className="text-sm text-zinc-300 hover:text-zinc-100">¿Olvidaste tu clave?</a>
                </div>
              )}

              {err && (
                <div className="rounded-md border border-red-900/40 bg-red-950/40 px-3 py-2 text-xs font-semibold text-red-300">
                  {err}
                </div>
              )}
          ...
          <CardFooter className="flex flex-col items-center gap-3 text-sm text-zinc-400">
            <div>
              ...
              >
                {mode === "login" ? "Registrarte" : "Ingresar"}
              </button>
            </div>
